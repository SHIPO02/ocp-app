...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # --- Initialisation du client Anthropic sécurisée ---
    try:
        # Cherche la clé dans les secrets Streamlit
        api_key = st.secrets["ANTHROPIC_API_KEY"]
        client = _anthropic.Anthropic(api_key=api_key)
    except Exception:
        st.error("🔑 Clé API Anthropic manquante. Configurez 'ANTHROPIC_API_KEY' dans les Secrets Streamlit.")
        st.stop()

    st.markdown('<div class="stitle purple">Pipeline des Ventes — Intelligence Artificielle</div>', unsafe_allow_html=True)

    # --- ZONE D'UPLOAD ---
    with st.container():
        st.markdown('<div class="upload-zone">', unsafe_allow_html=True)
        file_v = st.file_uploader("Charger le Pipeline Excel", type=EXCEL_T, key="v_up")
        if file_v:
            if st.button("🚀 Analyser avec l'IA"):
                with st.spinner("Analyse de la structure..."):
                    raw_v = file_v.read()
                    sheets, _, _ = read_excel_raw(raw_v, file_v.name)
                    mapping = llm_detect_columns({k: v for k, v in list(sheets.items())[:3]})
    ...
    if v_df is not None and not v_df.empty:
        ...
        # 1. Barre de Filtres Avancés
        st.markdown('<div class="filter-panel">', unsafe_allow_html=True)
        f1, f2, f3 = st.columns(3)
        
        with f1:
            # Filtrage par Site (Si la colonne existe ou via recherche textuelle)
            site_options = ["Tous", "Jorf", "Safi"]
            sel_site = st.selectbox("📍 Filtrer par Site", site_options)
            
        with f2:
            # Filtrage par Confirmation / Statut
            confirm_options = ["Tous"] + sorted(v_df["Status"].unique().tolist())
            sel_confirm = st.selectbox("✅ Statut Confirmation", confirm_options)
            
        with f3:
            # Filtrage par Mois
            col_t = st.session_state.get("ventes_mapping", {}).get("month_col", v_df.columns[0])
            mois_options = ["Tous"] + v_df["Mois"].unique().tolist()
            sel_mois = st.selectbox("📅 Sélection du Mois", mois_options)
        ...
        if sel_site != "Tous":
            # On cherche le nom du site dans toutes les colonnes pour être flexible
            mask = df_final.astype(str).apply(lambda x: x.str.contains(sel_site, case=False)).any(axis=1)
            df_final = df_final[mask]
            
        if sel_confirm != "Tous":
            df_final = df_final[df_final["Status"] == sel_confirm]
            
        if sel_mois != "Tous":
            df_final = df_final[df_final["Mois"] == sel_mois]

        # 3. Affichage des résultats
        if not df_final.empty:
            total_v = round(df_final["Total"].sum(), 1)
            st.metric("Volume Total Filtré", f"{total_v} KT")
            st.dataframe(df_final, use_container_width=True, hide_index=True)
            
            # Graphique simple
            fig = go.Figure()
            fig.add_trace(go.Bar(x=df_final["Mois"], y=df_final["Total"], marker_color="#6B3FA0"))
            fig.update_layout(**PL, title="Volumes par période")
            st.plotly_chart(fig, use_container_width=True)
        else:
