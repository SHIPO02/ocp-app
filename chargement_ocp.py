...
if page=="accueil":
    ...
    with h2:
        ...
        <div class="hero" style="height:100%;justify-content:center;display:flex;flex-direction:column;text-align:center">
          ...
          <div class="hero-stat-lbl">Production Totale Cumulée — Jorf + Safi</div>
    ...
    for col,title,desc,status,nav_key in modules:
        with col:
            ...
            if status=="active":
                if st.button(f"Accéder au module",key=f"open_{nav_key}",use_container_width=True):
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if any_data:
        ...
        # ── Ordre des colonnes :
        # Date | J_Eng | J_Cam | J_VL | RADE | S_Eng | S_VL | J_TOT | S_TOT | TOTAL
        co=["Date"]
        if jf is not None: co+=["J_Eng","J_Cam","J_VL"]
        if rf is not None: co+=["RADE"]          # Rade juste après le détail Jorf
        if sf is not None: co+=["S_Eng","S_VL"]
        if jf is not None: co+=["J_TOT"]
        if sf is not None: co+=["S_TOT"]
        co+=["TOTAL"]
        ...
        for col in udf.columns:
            ...
            elif col=="RADE": tr[col]=None   # Rade = pas de cumul
            else: tr[col]=round(udf[col].sum(),1)
        disp=pd.concat([udf,pd.DataFrame([tr])],ignore_index=True)

        # ── Noms exacts des colonnes
        # Ordre demandé : Date | Jorf (Export Engrais, Export Camions, VL Camions, Rade Jorf) | Safi (Export Engrais, VL Camions) | Total Jorf | Total Safi | TOTAL
        nm={
            "J_Eng":"Export Engrais",
            "J_Cam":"Export Camions",
            "J_VL" :"VL Camions",
            "RADE" :"Rade Jorf",
            "S_Eng":"Export Engrais",
            "S_VL" :"VL Camions",
            "J_TOT":"Total Jorf",
            "S_TOT":"Total Safi",
            "TOTAL":"TOTAL"
        ...
        for col,n in nm.items():
            if col in disp.columns: cfg[col]=st.column_config.NumberColumn(n,format="%.1f")

        # ── Barre d'en-têtes groupés colorée au-dessus du tableau
        # Ordre : JORF LASFAR (détail + Rade) | SAFI | TOTAL JORF | TOTAL SAFI | TOTAL
        n_jorf_det = sum(1 for col in ["J_Eng","J_Cam","J_VL"] if col in udf.columns)
        n_rade     = 1 if "RADE"  in udf.columns else 0
        n_safi_det = sum(1 for col in ["S_Eng","S_VL"] if col in udf.columns)
        n_tot_j    = 1 if "J_TOT" in udf.columns else 0
        n_tot_s    = 1 if "S_TOT" in udf.columns else 0
        n_total    = 1

        # Jorf = détail Jorf + Rade → regroupés sous "JORF LASFAR"
        n_jorf_grp = n_jorf_det + n_rade

        def grp(label, n_cols, bg, fg="#FFFFFF"):
            if n_cols == 0: return ""
            return (f'<div style="flex:{n_cols};background:{bg};color:{fg};text-align:center;'
                    f'padding:6px 4px;font-family:Barlow Condensed,sans-serif;font-size:12px;'
                    f'font-weight:700;letter-spacing:1.5px;text-transform:uppercase;'
                    f'border-radius:4px;margin:0 2px">{label}</div>')

        header_html = '<div style="display:flex;gap:0;margin-bottom:5px;padding-left:96px">'
        if n_jorf_grp: header_html += grp("Jorf Lasfar", n_jorf_grp, "#00843D")
        if n_safi_det: header_html += grp("Safi",        n_safi_det, "#1565C0")
        if n_tot_j:    header_html += grp("Total Jorf",  n_tot_j,    "#005C2A")
        if n_tot_s:    header_html += grp("Total Safi",  n_tot_s,    "#0D47A1")
        if n_total:    header_html += grp("Total",       n_total,    "#C05A00")
        header_html += '</div>'
        ...
        def copy_btn(container,df,col,lbl,key):
            ...
            with container:
                ...
                """,height=40)
        if jf is not None and "J_TOT" in udf.columns: copy_btn(cb1,udf,"J_TOT","Total Jorf","j")
        if sf is not None and "S_TOT" in udf.columns: copy_btn(cb2,udf,"S_TOT","Total Safi","s")
        if "TOTAL" in udf.columns: copy_btn(cb3,udf,"TOTAL","Total Cumulé","t")
