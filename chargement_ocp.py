...
# ══════════════════════════════════════════════════════════════════════════════
# PAGE : PIPELINE DES VENTES (Version Intégrale - Tout est conservé)
# ══════════════════════════════════════════════════════════════════════════════
elif page == "ventes":
    ...
    def auto_map(df):
        ...
        for role, kws in [
            ...
            ("work_month", ["working", "month"]), ("del_month", ["deliv", "month"]),
            ("site", ["site"]), ("status", ["status", "planif"]),
            ("confirmation", ["confirm"]), ("pays", ["pays"]),
            ("produit", ["produit"]), ("loading_port", ["loading", "port"]),
        ]:
    ...
    st.markdown('<div class="upload-zone"><div class="zone-title">📂 Charger le fichier Pipeline</div>'
                '<div class="zone-desc">Fichier Excel (BL, Physical, Working, Delivery Month, D1, D2, D3...)</div>', unsafe_allow_html=True)
    ...
    # ─── MAPPING MANUEL (L'expander que tu voulais garder) ──────────────────
    with st.expander("⚙️ Vérifier / Ajuster le mapping des colonnes"):
        ROLES = {"bl_month":"BL Month","phys_month":"Physical Month","work_month":"Working Month","del_month":"Delivery Month",
                 "site":"Site","status":"Status Planif","confirmation":"Confirmation","pays":"Pays","produit":"Produit",
                 "d1":"D1","d2":"D2","d3":"D3","loading_port":"Loading Port"}
        new_map = {}
        opts = ["(non mappé)"] + df_raw.columns.tolist()
        cols_map = st.columns(4)
        for i, (rk, rl) in enumerate(ROLES.items()):
            cur = vmap.get(rk); idx = opts.index(cur) if cur in opts else 0
    ...
    MOIS_EN = ["All","January","February","March","April","May","June","July","August","September","October","November","December"]
    ...
    sel_p_opts = ["Tous"] + sorted(df_raw[vmap["pays"]].dropna().unique().tolist()) if vmap.get("pays") else ["Tous"]
    ...
    if sel_m != "Tous" and col_mois_ref:
        ...
        df_f = df_f[df_f[col_mois_ref].astype(str).str.contains(f"{sel_m}|{m_en}", case=False, na=False)]
    ...
    # ─── KPI CARDS ORANGE (Tes cards initiales) ────────────────────────────
    ...
    st.markdown(f"""<div style="background:linear-gradient(135deg,#6B3FA0,#4527A0);color:white;padding:14px 20px;border-radius:10px;margin:12px 0 20px 0;display:flex;justify-content:space-between;align-items:center"><span style="font-family:'Barlow Condensed',sans-serif;font-size:16px;font-weight:700;letter-spacing:.5px">TOTAL PIPELINE</span><span style="font-family:'Barlow Condensed',sans-serif;font-size:30px;font-weight:800">{fmt_kt(total_m)} KT</span></div>""", unsafe_allow_html=True)

    # ─── TABLEAU COMPLET ───────────────────────────────────────────────────
    st.markdown('<div class="stitle">Tableau complet — toutes colonnes</div>', unsafe_allow_html=True)
    st.dataframe(df_f, use_container_width=True, hide_index=True)

    # ─── GÉNÉRATEUR DE RAPPORT (Avec détails D1, D2, D3 demandés) ──────────
    st.markdown("---")
    st.markdown('<div class="stitle purple">📊 Rapport Automatique avec Détails Décades</div>', unsafe_allow_html=True)
    
    if st.button("🖨️ Générer le Rapport", type="primary"):
        # Header Rapport
        st.markdown(f"""<div style="background:linear-gradient(135deg,#00843D,#005C2A);color:white;padding:30px;border-radius:12px;margin-bottom:20px;text-align:center"><div style="font-size:32px;font-weight:900">RAPPORT ANALYTIQUE — {sel_m.upper()}</div></div>""", unsafe_allow_html=True)
        ...
        for statut in statuts:
            ...
            sd1, sd2, sd3 = clean_num(df_stat[v_d1]).sum(), clean_num(df_stat[v_d2]).sum(), clean_num(df_stat[v_d3]).sum()
            st.markdown(f"""<div style="background:#F2F4F7;border-left:5px solid #12202E;padding:15px;margin-top:20px;border-radius:8px"><div style="display:flex;justify-content:space-between;font-weight:800;font-size:18px"><span>📌 {statut}</span><span>{fmt_kt(sd1+sd2+sd3)} KT &nbsp; <small style="font-size:12px;color:#64748B;font-weight:400">(D1:{fmt_kt(sd1)} D2:{fmt_kt(sd2)} D3:{fmt_kt(sd3)})</small></span></div></div>""", unsafe_allow_html=True)
            ...
            if c_port_site:
                for port_val in sorted(df_stat[c_port_site].dropna().unique()):
                    ...
                    pd1, pd2, pd3 = clean_num(df_port[v_d1]).sum(), clean_num(df_port[v_d2]).sum(), clean_num(df_port[v_d3]).sum()
                    st.markdown(f"""<div style="margin:10px 0 0 30px;padding:12px;background:white;border:1px solid #E2E8F0;border-left:4px solid #1E293B;border-radius:6px"><div style="display:flex;justify-content:space-between;font-weight:700;font-size:14px"><span>🚢 PORT : {norm_site(port_val)}</span><span>{fmt_kt(pd1+pd2+pd3)} KT &nbsp; <small style="font-size:11px;color:#94A3B8;font-weight:400">(D1:{fmt_kt(pd1)} D2:{fmt_kt(pd2)} D3:{fmt_kt(pd3)})</small></span></div></div>""", unsafe_allow_html=True)
                    ...
                    if vmap.get("pays"):
                        for pays_val in sorted(df_port[vmap["pays"]].dropna().unique()):
                            ...
                            pyd1, pyd2, pyd3 = clean_num(df_pays[v_d1]).sum(), clean_num(df_pays[v_d2]).sum(), clean_num(df_pays[v_d3]).sum()
                            st.markdown(f"""<div style="margin:4px 0 0 50px;padding:8px;background:#F8FAFC;border-radius:4px"><div style="display:flex;justify-content:space-between;font-size:12px;font-weight:600"><span>🌍 {pays_val}</span><span>{fmt_kt(pyd1+pyd2+pyd3)} KT &nbsp; <small style="font-size:10px;color:#94A3B8;font-weight:400">(D1:{fmt_kt(pyd1)} D2:{fmt_kt(pyd2)} D3:{fmt_kt(pyd3)})</small></span></div>""", unsafe_allow_html=True)
                            
                            if vmap.get("produit"):
                                for pr in sorted(df_pays[vmap["produit"]].dropna().unique()):
                                    df_pr = df_pays[df_pays[vmap["produit"]].astype(str).str.strip() == str(pr)]
                                    prd1, prd2, prd3 = clean_num(df_pr[v_d1]).sum(), clean_num(df_pr[v_d2]).sum(), clean_num(df_pr[v_d3]).sum()
                                    st.markdown(f'<div style="display:flex;justify-content:space-between;margin-left:15px;font-size:11px;color:#475569;border-bottom:1px solid #F1F5F9"><span>📦 {pr}</span><span><b>{fmt_kt(prd1+prd2+prd3)} KT</b> <small>(D1:{fmt_kt(prd1)} D2:{fmt_kt(prd2)} D3:{fmt_kt(prd3)})</small></span></div>', unsafe_allow_html=True)
                            st.markdown("</div>", unsafe_allow_html=True)

        st.markdown(f"""<div style="margin-top:30px;background:#12202E;color:white;padding:25px;border-radius:12px;text-align:right;font-size:36px;font-weight:900">TOTAL GÉNÉRAL : {fmt_kt(total_m)} KT</div>""", unsafe_allow_html=True)
# ══════════════════════════════════════════════════════════════════════════════
