    ...
    with st.status("🚀 Traitement des flux OCP en cours...", expanded=False) as status:
        try:
            ...
            if final_list:
                ...
                # ── Tableau interactif avec sélection de colonne ──────────────
                st.info("💡 **Astuce :** Cliquez sur l'en-tête **TOTAL** pour sélectionner toute la colonne → puis **Ctrl+C** pour copier.")

                # Formatage : entiers sans décimales, séparateur espace
                fmt = {c: "{:,.0f}".replace(",", " ") for c in ["Export Engrais", "Export Camions", "VL Camions", "TOTAL"]}

                st.dataframe(
                    show_df,
                    use_container_width=True,
                    hide_index=True,
                    height=min(600, 45 + 35 * len(show_df)),
                    column_config={
