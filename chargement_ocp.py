...
# ── MODIFICATION ICI POUR LES VIRGULES ──
def fmt(n): 
    # Formate avec virgule décimale et espace pour les milliers
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if any_data:
        ...
        # Streamlit format virgule natif dans st.dataframe
        st.dataframe(disp,use_container_width=True,hide_index=True,
                     height=min(660,45+35*len(disp)),
                     column_config=cfg)
        ...
        def copy_btn(container, df, col, lbl, key):
            ...
            # On remplace aussi par des virgules pour le copier-coller (Excel FR)
            txt = "\t".join(str(round(float(v), 1)).replace(".",",") for v in vals)
            bid = f"cb_{key}"
            with container:
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    def show_sim(dates,sv,na,nq,titre,seuil=36000):
        ...
        m1.metric("Stock minimum",f"{mn:,.0f} T".replace(".",","),f"le {mn_d.strftime('%d/%m/%Y')}")
        m2.metric("Stock final",f"{sv[-1]:,.0f} T".replace(".",","))
    ...
    with tab_jo:
        ...
        if mj=="ACS":
            ...
            def rl(lignes,pfx,cad_def):
                ...
                for i,t in enumerate(st.tabs([f"{l}" for l in lignes])):
                    with t:
                        ...
                        tot+=pl; st.info(f"**{la}** : {pl:,.0f} T".replace(".",","))
                return tot
            ...
            r1.metric("Production ACS",f"{pa:,.0f} T".replace(".",",")); r2.metric("Production ACP29",f"{pp:,.0f} T".replace(".",","))
            r3.metric("Conso ACP29 (x3.14)",f"{c29:,.0f} T".replace(".",",")); r4.metric("Stock final",f"{sf2:,.0f} T".replace(".",","),delta="Excédent" if sf2>0 else "Déficit")
